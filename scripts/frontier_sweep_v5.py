...
def main() -> int:
    ...
    total_runs = len(param_sets) * len(windows) * len(selected_scenarios)
    logger.info(
        "Benchmark frontier sweep: %s param sets × 1 strategy × windows=%s × scenarios=%s = %s runs sampled=%s",
        len(param_sets),
    ...
    print(
        ...
        f"{len(windows)} windows × {len(selected_scenarios)} scenarios = {total_runs} runs"
    )
    ...
    if args.resume and checkpoint:
        ...
        if mismatch:
            ...
            logger.error(
                "frontier_v5_resume_checkpoint_mismatch strategy=%s run_id=%s mismatch=%s",
                TARGET_STRATEGY,
                run_token,
                mismatch,
            )
